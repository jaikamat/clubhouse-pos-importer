...
describe('Receving workflow', () => {
    ...
    test('Add credit card', async () => {
        ...
        await page.waitFor(250);
        const receivingListItems = await page.$$eval(ui.receivingListItems, li => {
            return li.map(el => el.textContent);
        });
        expect(receivingListItems.length).toBe(3);
        const creditTotal = await page.$eval(ui.receivingCreditTotal, d => d.textContent);
    ...
    test('Add cash card', async () => {
        ...
        const cashTotal = await page.$eval(ui.receivingCashTotal, d => d.textContent);
...
describe('Manage Inventory totals confirmation', () => {
    ...
    test('Ensure correct number of cards were added', async () => {
        const nonfoilLabelQty = await page.$eval(ui.manageInvNonfoilLabel, l => l.textContent);
